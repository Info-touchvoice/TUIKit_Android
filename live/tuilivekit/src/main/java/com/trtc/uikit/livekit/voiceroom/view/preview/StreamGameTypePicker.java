...
public class StreamGameTypePicker extends PopupDialog {

    private       ListView  mListView;
    private final RoomState mRoomState;

    public StreamGameTypePicker(Context context, VoiceRoomManager liveController) {
        super(context);
    ...
    private void initListItemClickListener() {
        mListView.setOnItemClickListener((parent, view, position, id) -> {
            mRoomState.liveExtraInfo.gameType.setValue(RoomState.LiveStreamGameType.values()[position]);
            dismiss();
        });
    }

    private List<String> getDataList(Context context) {
        return Arrays.asList(context.getResources().getStringArray(R.array.common_stream_game_type));
    }
}
